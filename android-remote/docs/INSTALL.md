...
| Android | Android 8.0 (API 26) 及以上，需授权短信和存储权限 |
...
2. 首次启动会弹出**权限请求**，请全部允许：
   - **短信权限** — 用于远程读取短信
   - **存储/媒体权限** — 用于远程读取相册照片
3. **服务器地址** 填写：`ws://你的VPS_IP:3000/ws`
4. **设备名称** 随意填写（用于在控制台识别）
5. 点击 **启动 Agent**

> ⚠️ 如果不小心拒绝了权限，可到系统 **设置 → 应用 → RemoteAgent → 权限** 手动开启
...
| 启动 App | 通过包名启动目标应用 |
| 文件管理 | 浏览设备文件目录（需存储权限） |
| 📱 短信读取 | 远程获取设备收件箱短信，显示发件人、内容、时间 |
| 🖼️ 相册读取 | 远程浏览设备相册列表，点击可预览图片（<5MB） |
| 📷 屏幕截图 | 远程截取设备当前屏幕画面（部分设备需 ROOT） |

### 新功能使用说明

#### 短信读取
1. 在控制台选中目标设备
2. 找到「📱 短信读取」面板，点击 **获取短信**
3. 将显示最近 50 条收件箱短信（发件人、内容、时间）

> 需要 Android 端已授予 **READ_SMS** 权限

#### 相册读取
1. 找到「🖼️ 相册读取」面板，点击 **获取相册**
2. 显示设备最近 50 张照片列表（文件名、大小、时间）
3. 点击照片条目可远程**预览原图**（自动传输，限 5MB 以内）

> 需要 Android 端已授予 **READ_MEDIA_IMAGES**（Android 13+）或 **READ_EXTERNAL_STORAGE** 权限

#### 屏幕截图
1. 找到「📷 屏幕截图」面板，点击 **截取屏幕**
2. 稍等几秒后将在面板中显示设备当前屏幕画面

> ⚠️ 屏幕截图使用系统 `screencap` 命令，部分设备可能需要 ROOT 权限才能工作
...
**Q: 短信/相册获取为空？**
- 确认手机上已授予 RemoteAgent 对应权限（短信/存储/媒体）
- Android 13+ 使用 `READ_MEDIA_IMAGES` 替代旧存储权限
- 到系统 **设置 → 应用 → RemoteAgent → 权限** 检查

**Q: 屏幕截图显示 ERROR？**
- `screencap` 命令在部分非 ROOT 设备上可能无权限
- 已 ROOT 设备通常无此问题
- 可在 Shell 终端手动测试：`screencap -p /sdcard/test.png`

**Q: 相册图片加载失败？**
- 超过 5MB 的图片不会传输，会提示文件过大
- WebSocket 传输大图片时可能需要几秒，请耐心等待
